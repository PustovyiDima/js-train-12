...
function deleteKey(dictionary, key) {
   ...
   return dictionary.has(key) ? dictionary.delete(key) : false;
}
...
function getKeysStartingWith(dictionary, letter) {
   ...
   for (const [key, value] of dictionary) {
      // console.log(key.startsWith(letter));
      // key[0] == letter
      key.startsWith(letter) ? ((arr[i] = key), i++) : false;
   }
   return arr;
}
...
function addKeyValuePairs(dictionary, entries) {
   ...
   entries.forEach((element) => {
      ...
      dictionary.has(key)
         ? rejected++
         : (dictionary.set(key, value), (added += 1));
   });
   const obj = {
...
function transformDictionary(dictionary) {
   ...
   const obj = { keys: keys, values: values, entries: entries };
   return obj;
}
...
function checkPresence(dictionary, keys) {
   ...
   keys.forEach((element) => {
      arr.push(dictionary.has(element) ? true : false);
   });
   return arr;
}
...
function sortByValues(dictionary) {
   ...
   values.sort(([key1, value1], [key2, value2]) => value2 - value1);
   const res = new Map(values);
   return res;
}
